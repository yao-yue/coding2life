...
Page({
  ...
  onLoad: function (options) {
    if (options.cityName) {
      wx.setStorageSync('rentCityName', options.cityName);
    }
    if(options.cityCode){
      wx.setStorageSync('rentCityCode', options.cityCode);
    }
    // if (options.keyWord) {
    //   wx.setStorageSync('rentkeyWord', options.keyWord);
    // }
    if (options.date) {
      wx.setStorageSync('rentBeginDate', options.beginDate);
      wx.setStorageSync('rentEndDate', options.endDate);
      wx.setStorageSync('rentDate', options.date);
    }
    this.setData({
      keyWord: wx.getStorageSync('rentkeyWord') ? wx.getStorageSync('rentkeyWord') : '',
      cityName: wx.getStorageSync('rentCityName') ? wx.getStorageSync('rentCityName') : '城市',
      date: wx.getStorageSync('rentDate') ? wx.getStorageSync('rentDate') : '日期',
    })
    this.data.cityCode = wx.getStorageSync('rentCityCode') ? wx.getStorageSync('rentCityCode') : '',
    this.data.beginDate = wx.getStorageSync('rentBeginDate') ? wx.getStorageSync('rentBeginDate') : '',
    this.data.endDate = wx.getStorageSync('rentEndDate') ? wx.getStorageSync('rentEndDate') : '',
    this.calculate();
    this.getList();
  },
  linkTo(e) {
    ...
    wx.navigateTo({
      url: `../rentDetail/index?id=${id}`
    })
  },
  linkToSearch(){
    wx.navigateTo({
      url: `../search/index?fromPage=rentList&keyWord=${this.data.keyWord}`
    })
  },
  keyBind(e) {
    this.setData({
    ...
    wx.setStorageSync('rentkeyWord', this.data.keyWord);
  },
  search(){
    this.setData({
      dataList: [],
  ...
  chooseCity() {
    wx.navigateTo({
      url: `../cityList/index?fromPage=rentList`
    })
  },
  getList() {
    wx.showLoading({
    ...
    let url = API.URL.CAR.FINDRENTINFO;
    let param = {
    ...
    if (this.data.keyWord) { param.rentNameLike = this.data.keyWord; } 
  ...
  submitDate() {
    ...
    this.getList();
    wx.setStorageSync('rentBeginDate', beginDate);
    wx.setStorageSync('rentEndDate', endDate);
    wx.setStorageSync('rentDate', date);
  },
  dateForm(num) {
    if (num > 9) {
      return num;
