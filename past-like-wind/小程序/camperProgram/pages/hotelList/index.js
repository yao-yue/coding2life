...
Page({
  ...
  onLoad: function (options) {
    if (options.cityName){
      wx.setStorageSync('hotelCityName', options.cityName);
    }
    if (options.cityCode) {
      wx.setStorageSync('hotelCityCode', options.cityCode);
    }
    if (options.keyWord){
      wx.setStorageSync('hotelkeyWord', options.keyWord);
    }
    if(options.date){
      wx.setStorageSync('hotelBeginDate', options.beginDate);
      wx.setStorageSync('hotelEndDate', options.endDate);
      wx.setStorageSync('hotelDate', options.date);
    }
    this.setData({
      keyWord: wx.getStorageSync('hotelkeyWord') ? wx.getStorageSync('hotelkeyWord') : '',
      cityName: wx.getStorageSync('hotelCityName') ? wx.getStorageSync('hotelCityName') : '城市',
      date: wx.getStorageSync('hotelDate') ? wx.getStorageSync('hotelDate') : '日期',
      priceRange: wx.getStorageSync('hotelPrice') ? wx.getStorageSync('hotelPrice') : '价格',
    })
    this.data.cityCode = wx.getStorageSync('hotelCityCode') ? wx.getStorageSync('hotelCityCode') : '',
    this.data.value = wx.getStorageSync('hotelMaxPrice') ? wx.getStorageSync('hotelMaxPrice') : '2000',
    this.data.beginDate = wx.getStorageSync('hotelBeginDate') ? wx.getStorageSync('hotelBeginDate') : '',
    this.data.endDate = wx.getStorageSync('hotelEndDate') ? wx.getStorageSync('hotelEndDate') : '',
    this.calculate();
    this.getList();
  ...
  linkToSearch() {
    wx.navigateTo({
      url: `../search/index?fromPage=hotelList&keyWord=${this.data.keyWord}`
    })
  },
  keyBind(e) {
    this.setData({
    ...
    wx.setStorageSync('hotelkeyWord', this.data.keyWord);
  },
  search() {
    this.setData({
      dataList: [],
  ...
  linkTo(e) {
    ...
    let url = `../roomList/index?id=${id}`;
    if (this.data.beginDate && this.data.endDate) {
      url = `../roomList/index?id=${id}&date=${this.data.date}&beginDate=${this.data.beginDate}&endDate=${this.data.endDate}`;
    }
    wx.navigateTo({
      url: url
    })
  ...
  chooseCity(){
    wx.navigateTo({
      url: `../cityList/index?fromPage=hotelList`
    })
  },
  choosePrice(){
    this.setData({
  ...
  getPrice(){
    if(this.data.value == this.data.min){
      wx.showToast({ title:'请重新选择价格', image: '../../images/warn.png', duration: 1000 }); return false;
    }
    let priceRange = '￥0-￥' + this.data.value;
    this.setData({
      priceShow:false,
      priceRange,
      maxPrice: this.data.value,
      dataList:[],
      pageIndex:1,
      pageCount:0
    })
    this.getList();
    wx.setStorageSync('hotelPrice', this.data.priceRange)
    wx.setStorageSync('hotelMaxPrice', this.data.value)
  },
  getList() {
    wx.showLoading({
      title: '加载中...',
    ...
    let url = API.URL.CAR.FINDHOTELBYPAGE;
    let param = {
    ...
    if (this.data.cityCode) { param.cityCode = this.data.cityCode; }
    // if (!this.data.cityCode && this.data.cityName) { param.cityLike = this.data.cityName; } 
    if (this.data.keyWord) { param.nameLike = this.data.keyWord; }
  ...
  submitDate() {
    ...
    this.getList();
    wx.setStorageSync('hotelBeginDate', beginDate);
    wx.setStorageSync('hotelEndDate', endDate);
    wx.setStorageSync('hotelDate', date);
  },
  dateForm(num) {
    if (num > 9) {
      return num;
