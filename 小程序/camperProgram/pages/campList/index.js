...
Page({
  ...
  onLoad: function (options) {
    if (options.cityName) {
      wx.setStorageSync('campCityName', options.cityName);
    }
    if (options.cityCode) {
      wx.setStorageSync('campCityCode', options.cityCode);
    }
    if (options.keyWord) {
    ...
    this.setData({
      keyWord: wx.getStorageSync('campkeyWord') ? wx.getStorageSync('campkeyWord') : '',
      cityName: wx.getStorageSync('campCityName') ? wx.getStorageSync('campCityName') : '城市',
      date: wx.getStorageSync('campDate') ? wx.getStorageSync('campDate') : '日期',
    })
    this.data.cityCode = wx.getStorageSync('campCityCode') ? wx.getStorageSync('campCityCode') : '',
    this.data.beginDate = wx.getStorageSync('campBeginDate') ? wx.getStorageSync('campBeginDate') : '',
    this.data.endDate = wx.getStorageSync('campEndDate') ? wx.getStorageSync('campEndDate') : '',
  ...
  linkTo(e) {
    ...
    if (this.data.beginDate && this.data.endDate){
      url = `../carList/index?id=${id}&date=${this.data.date}&beginDate=${this.data.beginDate}&endDate=${this.data.endDate}`;
    }
    wx.navigateTo({
      url: url
    })
  ...
  linkToSearch() {
    wx.navigateTo({
      url: `../search/index?fromPage=campList&keyWord=${this.data.keyWord}`
    })
  },
  keyBind(e) {
    this.setData({
  ...
  chooseCity() {
    wx.navigateTo({
      url: `../cityList/index?fromPage=campList`
    })
  },
  getList() {
    wx.showLoading({
  ...
  submitDate() {
    ...
    this.getList();
    wx.setStorageSync('campBeginDate', beginDate);
    wx.setStorageSync('campEndDate', endDate);
    wx.setStorageSync('campDate', date);
  },
  dateForm(num) {
    if (num > 9) {
      return num;
  ...
  handleCalendar(e) {
    const handle = e.curcampTarget.dataset.handle;
